...
export default function LibrarianDashboard() {
  ...
  const { data: activeIssues, loading: activeLoading, error: activeError, refetch: refetchActive } = useApi(() => issueService.getActiveIssues(), [])
  const { data: overdueIssues } = useApi(() => issueService.getOverdueIssues(), [])
  const { data: reservations } = useApi(() => reservationService.getAllReservations(), [])
  const { data: returns } = useApi(() => returnService.getAllReturns(), [])
  const { data: copyReport } = useApi(() => reportService.getCopyReport(), [])
  const { data: fineReport } = useApi(() => reportService.getFineReport(), [])

  const dueToday = useMemo(
    () => (activeIssues || []).filter((i) => dueStatus(i.due_date).key === 'today'),
    [activeIssues]
  )
  ...
  const availableCopies = useMemo(
    () => (copyReport || []).filter((c) => c.availability_status === 'Available').length,
    [copyReport]
  )
  const outstandingFines = useMemo(
    () => (fineReport || []).filter((f) => f.payment_status === 'Pending').reduce((sum, f) => sum + Number(f.amount), 0),
    [fineReport]
  )

  const recentCirculation = useMemo(
    () => [...(activeIssues || [])].sort((a, b) => b.issue_id - a.issue_id).slice(0, 6),
    [activeIssues]
  )

  const displayName = user?.username || 'Librarian'
    ...
    <div className="ag">
      ...
      <aside className={`ag-sidebar ${mobileOpen ? 'is-open' : ''}`}>
        ...
        <nav className="ag-nav">
          ...
          <a className="ag-nav-item" onClick={() => navigate('/due-dates')}><i className="ti ti-clock-exclamation" />Due Dates</a>
      ...
      <main className="ag-main" id="lg-dash-main-content" tabIndex={-1}>
          ...
          <div className="ag-topbar-right">
            ...
            <div className="ag-profile">
              ...
              <div>
                ...
                <div className="ag-profile-role">Librarian</div>
              </div>
            </div>
          </div>
        </header>
        ...
        <section className="ag-tile-row">
          <div className="ag-tile tone-gold">
            <div className="ag-tile-top"><span>Currently issued</span><i className="ti ti-book-2" /></div>
            <div className="ag-tile-value">{activeLoading ? '—' : (activeIssues || []).length}</div>
            <div className="ag-tile-sub">{dueToday.length} due today</div>
          </div>
          <div className="ag-tile tone-copper">
            <div className="ag-tile-top"><span>Overdue</span><i className="ti ti-clock-exclamation" /></div>
            <div className="ag-tile-value">{(overdueIssues || []).length}</div>
            <div className="ag-tile-sub">Needs follow-up</div>
          </div>
          <div className="ag-tile tone-sage">
            <div className="ag-tile-top"><span>Available copies</span><i className="ti ti-books" /></div>
            <div className="ag-tile-value">{availableCopies}</div>
            <div className="ag-tile-sub">Ready to issue</div>
          </div>
          <div className="ag-tile tone-ink">
            <div className="ag-tile-top"><span>Outstanding fines</span><i className="ti ti-receipt" /></div>
            <div className="ag-tile-value">{formatCurrency(outstandingFines)}</div>
        ...
        <section className="ag-grid-2">
          <div className="ag-panel">
            <div className="ag-panel-head">
              <h2>Due today</h2>
              <button className="ag-panel-link" onClick={() => navigate('/due-dates')}>View all</button>
            </div>
            {activeLoading ? (
              <div className="ag-skeleton" style={{ height: 140 }} />
            ) : activeError ? (
              <div className="ag-empty-panel"><i className="ti ti-alert-triangle" /><p>Could not load due dates. <button className="ag-panel-link" onClick={refetchActive}>Retry</button></p></div>
            ) : dueToday.length === 0 ? (
              <div className="ag-empty-panel"><i className="ti ti-calendar-due" /><p>Nothing due today.</p></div>
            ) : (
              <table className="ag-table">
                ...
                <tbody>
                  {dueToday.map((i) => (
                    <tr key={i.issue_id}>
                      <td>
                        <div className="ag-book-title">{i.student_name}</div>
                        <div className="ag-book-sub">{i.roll_number}</div>
                      </td>
                      <td>{i.title}</td>
                      <td><StatusBadge status="Due today" tone="warning" /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="ag-panel">
            <div className="ag-panel-head">
              ...
              <button className="ag-panel-link" onClick={() => navigate('/reservations')}>View all</button>
            </div>
            {pendingReservations.length === 0 ? (
              <div className="ag-empty-panel"><i className="ti ti-bookmark" /><p>No reservations waiting.</p></div>
            ) : (
              <ul className="ag-activity-list">
                {pendingReservations.slice(0, 6).map((r) => (
                  <li className="ag-activity-row" key={r.reservation_id}>
                    <div className="ag-activity-icon"><i className="ti ti-bookmark" /></div>
                    <div>
                      <div className="ag-activity-text">Book #{r.book_id} — Student #{r.student_id}</div>
                      <div className="ag-activity-time">Reserved {formatDate(r.reservation_date)}</div>
                    </div>
                  </li>
                ))}
              </ul>
        ...
        <section className="ag-panel ag-panel-wide">
          ...
          {recentCirculation.length === 0 ? (
            ...
            <table className="ag-table">
              <thead><tr><th>Student</th><th>Book</th><th>Issued</th><th>Due</th><th>Status</th></tr></thead>
              <tbody>
                {recentCirculation.map((i) => {
                  ...
                  return (
                    <tr key={i.issue_id}>
                      ...
                      <td><StatusBadge status={s.label} tone={s.tone} /></td>
                    </tr>
                  )
                })}
              </tbody>
