...
export default function AdminDashboard() {
  ...
  const {
    ...
    error: dashError,
    refetch: refetchDash,
  } = useApi(() => reportService.getDashboardReport(), [])
  ...
  const {
    data: copyReport,
    error: copyError,
  } = useApi(() => reportService.getCopyReport(), [])

  const {
    data: issueReport,
    error: issueReportError,
  } = useApi(() => reportService.getIssueReport(), [])

  const { data: returnReport } = useApi(
    () => returnService.getAllReturns(),
    []
  )

  const { data: fines } = useApi(
  ...
  const monthlyIssues = useMemo(
    () => groupByMonth(issueReport || [], 'issue_date', 6),
    [issueReport]
  )

  const maxMonthly = Math.max(
    1,
    ...monthlyIssues.map((m) => m.v)
  )
  ...
  const copyStatus = useMemo(() => {
    ...
    for (const c of rows) {
      if (
        c &&
        counts[c.availability_status] !== undefined
      ) {
        counts[c.availability_status] += 1
      }
    }
  ...
  const totalCopies = copyStatus.reduce(
    (sum, c) => sum + c.value,
    0
  )

  const donutSegments = copyStatus.map((c, i) => {
    const prefix = copyStatus
      .slice(0, i)
      .reduce((sum, s) => sum + s.value, 0)

    const start = totalCopies
      ? (prefix / totalCopies) * 360
      : 0

    const end = totalCopies
      ? ((prefix + c.value) / totalCopies) * 360
      : 0
  ...
  const activity = useMemo(() => {
    ...
    for (const i of (issueReport || []).slice(0, 5)) {
      items.push({
        icon: 'ti-book-2',
        text: `Book issued — "${i.title || 'Unknown book'}" to ${
          i.student_name || 'Unknown student'
        }`,
        at: i.issue_date,
      })
    }
    ...
    for (const r of (returnReport || []).slice(0, 5)) {
      items.push({
        ...
        text: `Book returned — "${
          r.title || 'Unknown book'
        }", condition: ${r.condition_status || 'Unknown'}`,
        at: r.return_date || r.created_at,
      })
    }
    ...
    for (const f of (fines || []).slice(0, 5)) {
      items.push({
        icon: 'ti-receipt',
        text: `Fine of ${formatCurrency(
          f.amount || 0
        )} — ${f.payment_status || 'Unknown'}`,
        at: f.created_at,
      })
    }
    ...
    for (const res of (reservations || []).slice(0, 5)) {
      items.push({
        ...
        } — book #${res.book_id || 'N/A'}`,
        at: res.reservation_date,
      })
    }
          ...
          <section className="ag-tile-row">
            <div className="ag-tile tone-gold">
              ...
              <div className="ag-tile-value">
                {dashLoading
                  ? '—'
                  : (d.total_books || 0).toLocaleString(
                      'en-IN'
                    )}
              </div>

              <div className="ag-tile-sub">
                {(d.total_copies || 0).toLocaleString(
                  'en-IN'
                )}{' '}
                copies in catalog
              </div>
            ...
            <div className="ag-tile tone-sage">
              ...
              <div className="ag-tile-value">
                {dashLoading
                  ? '—'
                  : (d.active_students || 0).toLocaleString(
                      'en-IN'
                    )}
              </div>

              <div className="ag-tile-sub">
                {d.active_issues || 0} books currently issued
              </div>
            </div>
        ...
        <section className="ag-grid-2">
          ...
          <div className="ag-panel">
            ...
            {copyError ? (
              <div className="ag-empty-panel">
                <i className="ti ti-alert-triangle" />
                <p>Could not load copy status.</p>
              </div>
            ) : totalCopies === 0 ? (
              <div className="ag-empty-panel">
                <i className="ti ti-books" />
                <p>No book copies recorded yet.</p>
              </div>
            ) : (
                ...
                <div className="ag-donut-legend">
                  {copyStatus.map((c) => (
                    ...
                    >
                      ...
                      <span className="ag-legend-label">
                        {c.label}
                      </span>

                      <span className="ag-legend-value">
                        {c.value}
                      </span>
                    </div>
                  ))}
        ...
        <section className="ag-grid-2">
          <div className="ag-panel ag-panel-wide">
            ...
            {issuesLoading ? (
              ...
              <ErrorState
                message="Could not load recent circulation."
